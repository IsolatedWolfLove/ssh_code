...
- 终端运行在远端 tmux（或 screen）会话中，断线后训练任务继续运行，重连后可以重新接入原会话。
- 状态栏常驻远端主机资源：每张 GPU 的利用率、显存、温度和功耗，各卡上的进程归属，以及负载、内存和工作区所在文件系统的剩余空间。
- 远程图片（结果图、样本帧）可直接在标签页中预览缩放，并可开启自动刷新，边跑边看新产出的图。
- 终端工作目录会跟随当前工作区同步。
...
- 可选：远程机器安装 `tmux`（或 `screen`），让终端在断线后继续运行。
